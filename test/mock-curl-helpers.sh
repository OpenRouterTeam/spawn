#!/bin/bash
# Mock curl helper functions — extracted from test/mock.sh for readability
# These are sourced by the mock curl script to reduce heredoc nesting
...
# Inject mock error responses if configured
_maybe_inject_error() {
...
# Handle special URLs (installers, OpenRouter, github raw, etc)
...
# Strip cloud provider API base URLs to get just the endpoint
_strip_api_base() {
    ENDPOINT="$URL"
    case "$URL" in
...
# Check required fields are in JSON body
_check_fields() {
    local fields="$1"
...
# Validate POST body JSON and required fields
_validate_body() {
...
# Load fixture or return empty JSON
_try_fixture() {
...
# Return synthetic "active" server response for status polling
...
# Handle GET requests — try fixtures then synthetics
_respond_get() {
    local FIXTURE_NAME
...
# Handle POST requests — try fixtures or return synthetic responses
_respond_post() {
    case "$EP_CLEAN" in
...
# Track server creation/deletion for state verification
_track_state() {
