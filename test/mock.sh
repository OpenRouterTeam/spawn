...
# Run a script in background with mocked environment, enforcing a timeout.
# Sets LAST_EXIT_CODE in the caller's scope.
# Arguments: script_path cloud state_file fake_home
_run_script_with_timeout() {
    ...
    if [[ "$LAST_EXIT_CODE" -ne 124 ]]; then
        wait "$pid" 2>/dev/null || LAST_EXIT_CODE=$?
    fi
}

# Print last 20 lines of script output (called on failure)
...
# Assert error scenario: expect non-zero exit code.
# Writes result to RESULTS_FILE if set. Returns 0 always.
...
# Cloud-specific API call assertions (SSH key fetch + server creation)
...
# Write pass/fail result for this test to RESULTS_FILE (used by qa-dry-run.sh / qa-cycle.sh)
_record_test_result() {
    local cloud="$1"
    local agent="$2"
    local pre_failed="$3"
...
run_test() {
    ...
    # Reset mock log and set up environment
    : > "${MOCK_LOG}"
    ...
    # Run the script with mocked PATH + HOME
    ...
    # Error scenario mode: just check for expected failure and return
    if [[ -n "${MOCK_ERROR_SCENARIO:-}" ]]; then
        _assert_error_scenario "${exit_code}" "${cloud}" "${agent}"
        printf '\n'
        return 0
    fi
