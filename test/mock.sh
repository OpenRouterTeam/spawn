...
setup_mock_curl() {
    local helpers_file="${REPO_ROOT}/test/mock-curl-helpers.sh"
    local helpers_src

    # Read helpers file (must exist locally or we'll inline a fallback)
    if [ ! -f "$helpers_file" ]; then
        log_error "mock-curl-helpers.sh not found at $helpers_file"
        exit 1
    fi
    helpers_src=$(cat "$helpers_file")

    cat > "${TEST_DIR}/curl" << MOCKCURL
#!/bin/bash
# Mock curl — returns fixture data based on URL
# Env vars from parent: MOCK_LOG, MOCK_FIXTURE_DIR, MOCK_CLOUD, MOCK_REPO_ROOT

# Source helper functions (inlined for portability)
$helpers_src
