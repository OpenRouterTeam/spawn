...
if ! run_server "${OCI_SERVER_IP}" "command -v q &> /dev/null && q --version &> /dev/null"; then
    log_install_failed "Amazon Q CLI" "curl -fsSL https://desktop-release.q.us-east-1.amazonaws.com/latest/amazon-q-cli-install.sh | bash" "${OCI_SERVER_IP}"
    exit 1
fi
