...
log_info "Claude Code on Contabo Cloud"
echo ""
...
if ! run_server "${CONTABO_SERVER_IP}" "command -v claude" >/dev/null 2>&1; then
...
if ! run_server "${CONTABO_SERVER_IP}" "command -v claude &> /dev/null && claude --version &> /dev/null"; then
...
sleep 1
clear
interactive_session "${CONTABO_SERVER_IP}" "source ~/.zshrc && claude"
