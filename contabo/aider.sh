...
log_info "Aider on Contabo Cloud"
echo ""
...
run_server "${CONTABO_SERVER_IP}" "pip install aider-chat 2>/dev/null || pip3 install aider-chat"

# Verify installation succeeded
if ! run_server "${CONTABO_SERVER_IP}" "command -v aider &> /dev/null && aider --version &> /dev/null"; then
    log_error "Aider installation verification failed"
    log_error "The 'aider' command is not available or not working properly on server ${CONTABO_SERVER_IP}"
    exit 1
fi
...
sleep 1
clear
interactive_session "${CONTABO_SERVER_IP}" "source ~/.zshrc && aider --model openrouter/${MODEL_ID}"
