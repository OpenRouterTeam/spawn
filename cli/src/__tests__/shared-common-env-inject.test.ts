...
`);
    ...
    expect(result.stdout).toContain("/tmp/env_config");
    // inject_env_vars_local now appends to both .bashrc and .zshrc
    expect(result.stdout).toContain("RUN_ARGS: cat /tmp/env_config >> ~/.bashrc && cat /tmp/env_config >> ~/.zshrc && rm /tmp/env_config");
  });
