...
describe("Security Edge Cases", () => {
  describe("validateIdentifier boundary conditions", () => {
    ...
    it("should reject identifiers with dots", () => {
      expect(() => validateIdentifier("my.agent", "Test")).toThrow("Invalid");
    });
    ...
    it("should reject URL-like identifiers", () => {
      expect(() => validateIdentifier("http://evil.com", "Test")).toThrow("Invalid");
      expect(() => validateIdentifier("https://evil.com", "Test")).toThrow("Invalid");
    });

    it("should reject shell metacharacters individually", () => {
      const shellChars = ["!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "=", "+", "{", "}", "[", "]", "<", ">", "?", "~", "`", "'", "\"", ";", ",", "."];
      for (const char of shellChars) {
        expect(() => validateIdentifier(`test${char}name`, "Test")).toThrow("Invalid");
      }
    });
  });
...
`;
  ...
  describe("validatePrompt edge cases", () => {
    ...
    it("should reject backtick with complex commands", () => {
      expect(() => validatePrompt("Run `cat /etc/shadow`")).toThrow("backtick command substitution");
    });
