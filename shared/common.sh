...
_try_curl_installer() {
    ...
    if ${run_cb} "curl -fsSL https://claude.ai/install.sh | bash" 2>&1; then
        if _claude_code_is_installed "$run_cb"; then
...
install_claude_code() {
    ...
    if _try_curl_installer "$run_cb"; then
        _finalize_claude_install "$run_cb"
        return 0
    fi

    # Method 2: bun (requires Node.js)
    _ensure_node_runtime "$run_cb"
    if _try_bun_installer "$run_cb"; then
        _finalize_claude_install "$run_cb"
        return 0
    fi
