...
# Finalize Claude Code installation: run setup and write PATH to rc files
...
# Check if claude command is available in the given PATH
_claude_installed() {
    local run_cb="$1"
    local claude_path="$2"
...
# Try installing Claude Code via curl, finalize if successful
_install_via_curl() {
    local run_cb="$1"
    local claude_path="$2"
    ...
    if ${run_cb} "curl -fsSL https://claude.ai/install.sh | bash" 2>&1; then
        if _claude_installed "${run_cb}" "${claude_path}"; then
...
# Ensure Node.js is available (required for bun method)
...
# Try installing Claude Code via bun, finalize if successful
_install_via_bun() {
    local run_cb="$1"
    local claude_path="$2"
...
install_claude_code() {
    ...
    _install_via_curl "${run_cb}" "${claude_path}" && return 0

    # Ensure Node.js is available
    _ensure_node_runtime "${run_cb}" "${claude_path}"

    # Try bun method
    _install_via_bun "${run_cb}" "${claude_path}" && return 0
