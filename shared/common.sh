...
_report_api_failure() {
    ...
    if [[ "${retry_reason}" == "Cloud API network error" ]]; then
        log_error ""
        log_error "Could not reach the cloud provider's API."
        log_error ""
        log_error "How to fix:"
        log_error "  1. Check your internet connection: curl -s https://httpbin.org/ip"
        log_error "  2. Check DNS resolution: nslookup the provider's API hostname"
        log_error "  3. If behind a proxy or firewall, ensure HTTPS traffic is allowed"
        log_error "  4. Try again in a few moments (the API may be temporarily down)"
    else
        log_error ""
        log_error "This is usually caused by rate limiting or temporary provider issues."
