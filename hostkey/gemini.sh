...
log_info "Gemini CLI on HOSTKEY"
echo ""
...
run_server "${HOSTKEY_INSTANCE_IP}" "npm install -g @google/gemini-cli"

# Verify installation succeeded
if ! run_server "${HOSTKEY_INSTANCE_IP}" "command -v gemini &> /dev/null && gemini --version &> /dev/null"; then
    log_install_failed "Gemini CLI" "npm install -g @google/gemini-cli" "${HOSTKEY_INSTANCE_IP}"
    exit 1
fi
...
sleep 1
clear
interactive_session "${HOSTKEY_INSTANCE_IP}" "source ~/.zshrc && gemini"
